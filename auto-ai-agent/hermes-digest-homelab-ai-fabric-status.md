...
---
STRICT budget: at most 4 terminal calls, curl only. Probe each FQDN health endpoint with: curl -sS -m 8 -o /dev/null -w '%{http_code}' <url> . URLs: https://llm.pve.jacobpevans.com/v1/models ; https://hermes.pve.jacobpevans.com/ ; https://qdrant.pve.jacobpevans.com/ ; https://agentgateway.pve.jacobpevans.com/ . Final response UNDER 12 LINES: a plain-text component/status list with the ACTUAL http code per component (2xx/3xx=up, else down). Auto-delivered - no slack tool, no SILENT. Report exactly what curl returned. Never invent status. End with the model id.
